...
describe('PUT /rest/:id', () => {
    ...
    test('Vorhandene Packstation aendern', async () => {
        // given
    ...
    test('Nicht-vorhandene Packstation aendern', async () => {
        // given
        ...
        const { status }: AxiosResponse<string> = await client.put(
            url,
            geaendertePackstationIdNichtVorhanden,
            { headers },
        );
    ...
    test('Vorhandene Packstation aendern, aber mit ungueltigen Daten', async () => {
        // given
        const url = `/rest/${idVorhanden}`;
        const token = await loginRest(client);
        headers.Authorization = `Bearer ${token}`;
        ...
        const { status, data }: AxiosResponse<Record<string, any>> =
            await client.put(url, geaendertePackstationInvalid, { headers });
    ...
    test('Vorhandene Packstation aendern, aber ohne Versionsnummer', async () => {
        // given
        const url = `/rest/${idVorhanden}`;
        const token = await loginRest(client);
        headers.Authorization = `Bearer ${token}`;
    ...
    test('Vorhandene Packstation aendern, aber mit alter Versionsnummer', async () => {
        // given
        const url = `/rest/${idVorhanden}`;
        const token = await loginRest(client);
        headers.Authorization = `Bearer ${token}`;
    ...
    test('Vorhandene Packstation aendern, aber ohne Token', async () => {
        // given
        const url = `/rest/${idVorhanden}`;
    ...
    test('Vorhandene Packstation aendern, aber mit falschem Token', async () => {
        // given
        const url = `/rest/${idVorhanden}`;
        const token = 'FALSCH';
        headers.Authorization = `Bearer ${token}`;
